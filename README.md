...
**Project Flow Diagram (ASCII):**
```
User -> FastAPI (API Layer) -> Redis Queue -> Celery Worker
        |                                   |
        +------> Frontend UI <--------------+
Worker -> SpamModel -> Preprocessing -> Tokenizer -> Prediction
        -> Result stored in Redis -> API returns JSON/HTML
```
